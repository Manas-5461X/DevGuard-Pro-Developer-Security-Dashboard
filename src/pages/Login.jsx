...
function getFirebaseLoginError(err) {
  ...
  switch (code) {
    case 'auth/user-not-found':
      return { message: "No account found with this email.", action: 'signup', actionText: 'Create a new account' };
    ...
    case 'auth/too-many-requests':
      return { message: "Too many failed attempts. Please wait a few minutes before trying again.", action: null };
    case 'auth/user-disabled':
      return { message: "This account has been disabled. Please contact support.", action: null };
    case 'auth/network-request-failed':
      return { message: "Network error. Please check your connection and try again.", action: null };
    case 'auth/popup-closed-by-user':
      return { message: "Sign-in window was closed before completing. Please try again.", action: null };
...
export default function Login() {
  ...
  const [error, setError] = useState(null); // { message, action, actionText }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(getFirebaseLoginError(err));
    }
    setLoading(false);
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-cyber-bg px-4 py-10 font-sans">
      ...
      <div className="relative max-w-sm w-full">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-cyber-text tracking-tight mb-1">Welcome back</h1>
          <p className="text-cyber-dark-text text-sm">Sign in to DevGuard Pro</p>
        </div>
        ...
        {error && (
          <div className="mb-5 p-4 bg-cyber-error/10 border border-cyber-error/30 rounded-xl">
            ...
            {error.action === 'signup' && (
              <Link to="/signup" className="mt-2 flex items-center gap-1.5 text-cyber-primary text-xs font-semibold hover:underline">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <PasswordInput value={password} onChange={e => setPassword(e.target.value)} id="login-password" />
          </div>
          <button
            disabled={loading}
            type="submit"
        ...
        <div className="flex items-center my-6">
          ...
          <span className="px-3 text-cyber-dark-text text-xs">or continue with</span>
          <div className="flex-1 h-px bg-cyber-border" />
        </div>
        ...
        <p className="text-center text-cyber-dark-text mt-8 text-sm">
          ...
          <Link to="/signup" className="text-cyber-primary hover:text-cyber-primary-hover font-semibold transition-colors">
            Sign up free
          </Link>
        </p>
      </div>
